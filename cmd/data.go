...
var dataCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		for _, field := range viper.GetStringSlice("plenticore.fields") {
			...
			if len(p) != 2 {
				cmd.PrintErrf("Invalid field format: %s. Expected format is 'moduleID/fieldID'\n", field)
				return
			}
		...
		// Sort fields by moduleID and fieldIDs alphanumerically
		sort.Slice(fields, func(i, j int) bool {
			return fields[i].ModuleID < fields[j].ModuleID
		})

		// Sort each field's FieldIDs slice
		for i := range fields {
			sort.Strings(fields[i].FieldIDs)
		}

		data := commandCtx.Client.Data(fields)
